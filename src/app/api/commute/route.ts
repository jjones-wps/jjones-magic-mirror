...
async function fetchCommuteFromTomTom(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`TomTom API error for ${config.name}:`, response.status, await response.text());
      return null;
    }

    const data: TomTomRouteResponse = await response.json();
    const parsed = parseTomTomResponse(data, config.name, config.targetArrivalTime);
...
export async function GET() {
  ...
  const validConfigs = configs.filter((config) => config.origin && config.destination);
